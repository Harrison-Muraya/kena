...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateblockHeight(height) {
        ...
        if (el) el.textContent = height.toLocaleString();

    }

    // update Pending Transactions count display
    function updatePendingCount(count) {    
        const el = document.getElementById('pendingTx');
    ...
    function connectBlockSocket() {
        ...
        blockSocket.onmessage = e => {
            try {
                ...
                if (data.blocks[0]?.height) {
                    updateblockHeight(data.blocks[0].height);
                }
                container.innerHTML = '';
